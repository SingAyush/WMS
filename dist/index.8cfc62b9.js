...
// Loading the dataset
function add_column() {
    df.addColumn("year_added", df["date_added"].dt.year(), {
        inplace: true
    });
    // Plot
...
function handle_null() {
    let df_country = df["country"].valueCounts().sortValues({
        ascending: false
    });
    let values = [
        df_country["index"][0],
...
function fetch_top_10() {
    ...
    small.sortValues("release_year", {
        inplace: true
    });
    small.resetIndex({
        inplace: true
    });
    large.sortValues("release_year", {
        inplace: true
    });
    large.resetIndex({
        inplace: true
    });
    medium.sortValues("release_year", {
        inplace: true
    });
    medium.resetIndex({
        inplace: true
    ...
    div1.innerHTML = String(small.loc({
        columns: [
            "title",
            "release_year"
        ]
    ...
    div2.innerHTML = String(large.loc({
        columns: [
            "title",
            "release_year"
        ]
    ...
    div3.innerHTML = String(medium.loc({
        columns: [
            "title",
            "release_year"
        ]
...
dfd.readCSV("https://raw.githubusercontent.com/anmol5936/website-1/main/data.csv").then((data)=>{
    df = data;
    handle_null();
    content_type_plot();
    content_growth();
...
},{}]},["86oZd","1xC6H","6rimH"], "6rimH", "parcelRequire1530")
